    ...
    <section v-loading="pageLoading">
        ...
        <el-row :span="24" class="toolbar">
            <el-form :inline="true" :model="filters">
                <el-row>
                    <el-form-item>
                        <el-input class="search-input" v-model="filters.keyWord" placeholder="学员姓名、电话或身份证号码" icon="search" :on-icon-click="queryStudents"></el-input>
                    </el-form-item>
                    <el-form-item>
                        <el-input class="search-input" v-model="filters.recruitUser" placeholder="招生人" icon="search" :on-icon-click="queryStudents"></el-input>
                    </el-form-item>
                    <el-form-item>
                        <el-date-picker v-model="filters.daterange" type="daterange" placeholder="报名日期" :editable="false" @change="handleDateRangeChange"></el-date-picker>
                    </el-form-item>
                </el-row>
                <el-row>
                    <el-form-item>
                        <el-select v-model="filters.curStuType" style="width:150px;" placeholder="选择类型" @change="queryStudents">
                    ...
                    <el-form-item>
                        <el-select v-model="filters.isAppointment" style="width:150px;" placeholder="选择培训类型" @change="queryStudents" ref="mySel">
                    ...
                    <el-form-item>
                        <VTYPE :type="filters.curCarType" tag="vtype" v-on:child-emit="listenData" style="width:150px;"></VTYPE>
                    </el-form-item>
                    <el-form-item>
                        <el-select v-model="filters.uploadState" style="width:150px;" placeholder="选择备案状态" @change="queryStudents" ref="mySel">
                            <el-option label="全部备案状态" value=""></el-option>
                            <el-option label="未备案" value="0"></el-option>
                            <el-option label="备案成功" value="1"></el-option>
                            <el-option label="修改未备案" value="2"></el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item class="right">
                        <el-dropdown @command="handleCommand">
                            ...
                            <el-dropdown-menu slot="dropdown">
                                ...
                                <el-dropdown-item command="import" disabled>
                                    导入学员
                                    <!-- <a href="javascript:;" class="ml10" style="color:#20A0FF;" @click.stop="()=>{return false;}">下载模板</a> -->
                                </el-dropdown-item>
                    ...
                    <el-form-item class="right">
                        <el-button type="primary" @click="()=>{handleCommand('filing');}" :disabled="recordStudents.studentIdList.length===0">学员备案</el-button>
                    </el-form-item>
                    <el-form-item class="right">
                        <el-button type="primary" @click="handleShowNewWin">学员报名</el-button>
                    </el-form-item>
                </el-row>
            </el-form>
        ...
        <el-row :span="24">
            <el-table :data="students" highlight-current-row @selection-change="handleSelectionChange" @row-click="rowClick">
                ...
                <el-table-column prop="cardNo" label="身份证号码" min-width="150">
                </el-table-column>
                <el-table-column prop="carTypeName" :label="carTypeBySearchText">
                    <!-- :filters="carTypeOptionsBySearch" :filter-multiple="false" :filter-method="handleFilter" filter-placement="bottom-end" -->
                </el-table-column>
                <el-table-column prop="stage2Progress" label="科二进度">
                </el-table-column>
                <el-table-column prop="stage3Progress" label="科三进度">
                </el-table-column>
                <el-table-column prop="schoolName" label="分校" width="110" :show-overflow-tooltip="true">
                </el-table-column>
                <el-table-column prop="recruitUser" label="招生人">
                </el-table-column>
                <el-table-column prop="transactor" label="办理人">
                </el-table-column>
                <!--<el-table-column prop="reportState" label="上报状态" :formatter="formatter"></el-table-column>-->
                <el-table-column prop="signTime" label="报名日期" min-width="100">
                </el-table-column>
                <el-table-column prop="address" label="联系住所地址" width="120" :show-overflow-tooltip="true">
                </el-table-column>
        ...
        <!--高拍仪拍照-->
        <el-dialog title="照片" v-model="photoFormVisible" :close-on-click-modal="false" size="small" @close="dialogClose('high')" @open="dialogOpen" :before-close="dialogBeforeClose">
        ...
        <!--学员头像拍照-->
        <el-dialog title="拍照" v-model="photographFormVisible" :close-on-click-modal="false" class="photograph" :size="photographFormSize" @open="dialogOpen" @close="dialogClose('captrue_info')" :before-close="dialogBeforeClose">
                    ...
                    <el-row>
                        ...
                        <el-col :span="14">
                            <p class="t-right">
                                ...
                                <el-button v-if="detailsForm.applicationformState===40" type="primary" size="large" slot="reference" style="color:#bfcbd9;cursor:not-allowed;background-image:none;background-color:#eef1f6;border-color:#d1dbe5;" v-popover:applicationformFailReason>打印申请表</el-button>
                            </p>
                        </el-col>
                    </el-row>
                    <el-row>
...
export default {
    data() {
        return {
            ...
            carTypeOptions: [],
            carTypeOptionsBySearch: [],
            carTypeBySearchText: "车型",
            registProvinceData: [],
            registCityData: [],
            registCountyData: [],
            departmentData: [],
    ...
    methods: {
        ...
        queryStudents() {
            ...
            if (this.filters.daterange.length) {
                beginDate = daterange[0] === null ? "" : new Date(daterange[0]).Format("yyyy-MM-dd");
                endDate = daterange[1] === null ? "" : new Date(daterange[1]).Format("yyyy-MM-dd");
            }
            let args = [this.schoolCode, this.page, this.pageSize, this.filters.keyWord, this.filters.curCarType, this.filters.isAppointment, this.filters.curStuType, beginDate, endDate, this.filters.recruitUser, this.filters.uploadState];
            setTimeout(() => {
                request.basic.student.query.list(args).then((res) => {
                    ...
                    if (code === 200) {
                        ...
                        if (_res.success) {
                            ...
                            this.total = _res.object.num;
                            this.students = _res.object.list;
                            //studentName phone cardNo carTypeName schoolName stage2Progress stage3Progress uploadState reportState signTime
                        }
                    }
                    else {
                        this.pageLoading = false;
            ...
            request.public.queryCarType().then((res) => {
                if (res.success === true) {
                    ...
                    for (let item in data) {
                        ...
                        this.carTypeOptionsBySearch.push({
        ...
        dialogBeforeClose(done) {
            ...
            if (captrue && typeof captrue.bStopPlay !== "undefined") {
                //停止运行高拍仪
                captrue.bStopPlay();
                console.log("高拍仪设备已停止");
            }
            done();
        },
        //Dialog被关闭回调
        ...
        readMachine(type) {
            readInterval = 0;
            let browser = global.browser.check();
            if (type === "id") {
                try {
                    ...
                    this.$message.error(browser !== "IE" ? "当前浏览器不支持身份证读卡设备，建议更换IE9以上版本浏览器访问" : "身份证读卡设备驱动未安装或安装未成功");
                    return;
                }
                ...
                let ShenFenZhengResult = ShenFenZheng.ReadCard("1001", "d:\\test\\test.jpg");
                ...
                if (typeof (CVR_IDCard.ReadCard) === "undefined") {
                    this.$message.error("当前浏览器不支持高拍仪设备读取信息，建议更换IE9以上版本浏览器访问");
                }
                else {
        ...
        handleFilter(value, row) {
            console.log(value);
            this.filters.curCarType = value;
            this.queryStudents();
            if (value !== 0) {
                let data = this.carTypeOptionsBySearch.filter((item) => {
                    return item.value === value ? item : "";
                });
                this.carTypeBySearchText = "车型 " + data[0].text;
            }
            else {
                this.carTypeBySearchText = "车型";
            }
        },
